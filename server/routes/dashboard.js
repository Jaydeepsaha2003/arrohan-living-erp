...
router.get(
  ...
  wrap((req, res) => {
    ...
    // -------------------------------------------------------------- today
    const todayStats = {
      enquiries: one(`SELECT COUNT(*) n FROM enquiries WHERE date(enquiry_date) = ?`, t),
      quotations: one(`SELECT COUNT(*) n FROM quotations WHERE date(quotation_date) = ?`, t),
      salesOrders: one(`SELECT COUNT(*) n FROM sales_orders WHERE date(so_date) = ?`, t),
      deliveries: one(`SELECT COUNT(*) n FROM dispatches WHERE date(dispatch_date) = ?`, t),
      lost: one(`SELECT COUNT(*) n FROM enquiries WHERE status = 'lost' AND date(closed_at) = ?`, t),
      collected: oneVal(`SELECT COALESCE(SUM(amount),0) v FROM payments WHERE date(received_at) = ?`, t),
      invoiced: oneVal(`SELECT COALESCE(SUM(grand_total),0) v FROM invoices WHERE date(invoice_date) = ?`, t),
    };

    // ------------------------------------------------------------- month
    const monthStats = {
      enquiries: one(`SELECT COUNT(*) n FROM enquiries WHERE date(enquiry_date) >= ?`, monthStart),
      converted: one(`SELECT COUNT(*) n FROM enquiries WHERE status='converted' AND date(enquiry_date) >= ?`, monthStart),
      lost: one(`SELECT COUNT(*) n FROM enquiries WHERE status='lost' AND date(enquiry_date) >= ?`, monthStart),
      invoiced: oneVal(`SELECT COALESCE(SUM(grand_total),0) v FROM invoices WHERE date(invoice_date) >= ?`, monthStart),
      collected: oneVal(`SELECT COALESCE(SUM(amount),0) v FROM payments WHERE date(received_at) >= ?`, monthStart),
      delivered: one(`SELECT COUNT(*) n FROM dispatches WHERE date(dispatch_date) >= ?`, monthStart),
    };
    ...
    // ----------------------------------------------------------- headline
    const openEnquiries = one(`SELECT COUNT(*) n FROM enquiries WHERE status = 'open'`);
    const activeOrders = one(`SELECT COUNT(*) n FROM orders WHERE status = 'active'`);
    const onHold = one(`SELECT COUNT(*) n FROM orders WHERE status = 'active' AND hold = 1`);
    const closedOrders = one(`SELECT COUNT(*) n FROM orders WHERE status = 'closed'`);
    const lostOrders = one(`SELECT COUNT(*) n FROM orders WHERE status = 'lost'`);
    ...
    const stockValue = oneVal(`SELECT COALESCE(SUM(qty_in_stock * standard_rate),0) v FROM materials WHERE active = 1`);
    const fgUnits = oneVal(`SELECT COALESCE(SUM(qty),0) v FROM finished_goods WHERE status = 'in_stock'`);

    // --------------------------------------------------- my work queue
    const myStages = wf.stagesForRole(req.user.role);
    const myQueue = myStages.length
      ? db
          .prepare(
    ...
    const overdue = db
      ...
      .all()
      .map((r) => ({ ...r, stageLabel: wf.stageLabel(r.current_stage) }));
    ...
    const recentActivity = db
      ...
      .all()
      .map((r) => ({ ...r, stageLabel: wf.stageLabel(r.stage) }));
...
function oneVal(sql, ...args) {
  return num(db.prepare(sql).get(...args).v);
}
