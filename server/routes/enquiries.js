...
router.get(
  '/:id',
  wrap(async (req, res) => {
    const enq = await db.get('SELECT * FROM enquiries WHERE id = ?', Number(req.params.id));
    if (!enq) throw http(404, 'Enquiry not found.');
    enq.items = await db.all('SELECT * FROM enquiry_items WHERE enquiry_id = ? ORDER BY seq, id', enq.id);
    enq.order = enq.order_id
      ? await db.get('SELECT id, order_no, current_stage, status FROM orders WHERE id = ?', enq.order_id)
      : null;
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    const out = await db.tx(async () => {
      const enquiryNo = await db.nextDocNo('ENQ');
      let customerId = b.customer_id ? Number(b.customer_id) : null;
      if (!customerId && b.save_customer) customerId = await upsertCustomerFromEnquiry(b, req.user.id);
      ...
      await insertItems(enquiryId, items);

      let order = null;
      if (send) order = await convertToOrder(enquiryId, req.user);
      return { enquiryId, enquiryNo, order };
    });

    await db.audit(req, send ? 'enquiry.create+send' : 'enquiry.create', 'enquiry', out.enquiryId, {
...
router.patch(
  ...
  wrap(async (req, res) => {
    ...
    const enq = await db.get('SELECT * FROM enquiries WHERE id = ?', id);
    if (!enq) throw http(404, 'Enquiry not found.');
    ...
    if (b.expected_budget !== undefined) {
      sets.push('expected_budget = ?');
      vals.push(num(b.expected_budget));
    }
    if (b.enquiry_date !== undefined) {
      sets.push('enquiry_date = ?');
      vals.push(dateOnly(b.enquiry_date) || today());
    }
    sets.push(`updated_at = datetime('now')`);

    const out = await db.tx(async () => {
      await db.run(`UPDATE enquiries SET ${sets.join(', ')} WHERE id = ?`, ...vals, id);
      if (b.items !== undefined) {
        const items = normaliseItems(b.items);
        await db.run('DELETE FROM enquiry_items WHERE enquiry_id = ?', id);
        await insertItems(id, items);
      }
      return b.send ? convertToOrder(id, req.user) : null;
    });

    await db.audit(req, b.send ? 'enquiry.send' : 'enquiry.update', 'enquiry', id, { enquiry_no: enq.enquiry_no });
...
router.post(
  ...
  wrap(async (req, res) => {
    const id = Number(req.params.id);
    const order = await db.tx(() => convertToOrder(id, req.user));
    await db.audit(req, 'enquiry.send', 'enquiry', id, { order_no: order.order_no });
    res.json({ order });
  })
);
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    await db.audit(req, 'enquiry.lost', 'enquiry', id, { enquiry_no: enq.enquiry_no, reason, note });
    res.json({ ok: true });
  })
);
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    await db.audit(req, 'enquiry.reopen', 'enquiry', id, { enquiry_no: enq.enquiry_no });
    res.json({ ok: true });
  })
);
...
async function insertItems(enquiryId, items) {
  const SQL = `INSERT INTO enquiry_items (enquiry_id, seq, product, size, qty, unit, material, laminate, colour, hardware, remarks)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  for (const i of items) {
    await db.run(SQL, enquiryId, i.seq, i.product, i.size, i.qty, i.unit, i.material, i.laminate, i.colour, i.hardware, i.remarks);
  }
}

async function upsertCustomerFromEnquiry(b, userId) {
  const name = str(b.cust_name);
  if (!name) return null;
  const found = await db.get(
    `SELECT id FROM customers WHERE lower(trim(name)) = lower(trim(?)) AND IFNULL(phone,'') = IFNULL(?,'')`,
    name, str(b.cust_phone)
  );
 ...
 */
async function convertToOrder(enquiryId, user) {
  const enq = await db.get('SELECT * FROM enquiries WHERE id = ?', enquiryId);
  if (!enq) throw http(404, 'Enquiry not found.');
  ...
  const countRow = await db.get('SELECT COUNT(*) n FROM enquiry_items WHERE enquiry_id = ?', enquiryId);
  if (!countRow.n) throw http(400, 'Add at least one product line before sending to the factory.');

  const orderNo = await db.nextDocNo('ARL');
  const info = await db.run(
    `INSERT INTO orders (order_no, enquiry_id, current_stage, priority, created_by)
     VALUES (?, ?, 'costing', ?, ?)`,
    orderNo, enquiryId, 'Normal', user.id
  );
  ...
  await logStage(orderId, 'enquiry', 'completed', user, `Enquiry ${enq.enquiry_no} sent to Factory / Costing`);
